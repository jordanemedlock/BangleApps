...
function draw() {
  ...
  g.drawString(time, X, Y, false /*clear background*/);

  // Draw AM/PM
  if (is12Hour) {
    g.setFont("6x8", 1.5);
    g.setFontAlign(-1,1);
    g.drawString(ap, X+5, Y-25, false);
  }
  g.setFontAlign(1,1); // align right bottom

  // draw the seconds (2x size 7 segment)
  g.setFont("7x11Numeric7Seg",2);
  g.drawString(("0"+d.getSeconds()).substr(-2), X+30, Y, false /*clear background*/);
  // draw the date, in a normal font
  g.setFont("6x8", 2);
  g.setFontAlign(0,1); // align center bottom
  // pad the date - this clears the background if the date were to change length
  var weekday = "    "+require("locale").dow(d)+"    ";
  g.drawString(weekday, g.getWidth()/2, Y+27, false /*clear background*/);
  
  var dateStr = "    "+require("locale").date(d)+"    ";
  g.drawString(dateStr, g.getWidth()/2, Y+50, false /*clear background*/);
}
